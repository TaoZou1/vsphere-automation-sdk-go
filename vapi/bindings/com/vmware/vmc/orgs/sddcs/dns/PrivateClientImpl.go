 ...
 *
 * Client stubs for service: Private
 * Functions that implement the generated PrivateClient interface
 */
...
func NewPrivateClientImpl(connector client.Connector) *PrivateClientImpl {
      interfaceName := "com.vmware.vmc.orgs.sddcs.dns.private"
      ...
      pIface := PrivateClientImpl{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier:interfaceIdentifier, connector: connector}
...
func (pIface *PrivateClientImpl) Update(orgParam string, sddcParam string) (model.Task, error) {
...
func (pIface *PrivateClientImpl) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
    methodResult := pIface.connector.GetApiProvider().Invoke(pIface.interfaceName, methodId.Name(), inputDataValue, ctx)
    return methodResult
}

func (pIface *PrivateClientImpl) updateMethodDefinition() *core.MethodDefinition {
      interfaceIdentifier := core.NewInterfaceIdentifier(pIface.interfaceName)
      typeConverter := pIface.connector.TypeConverter()

      input, inputError := typeConverter.ConvertToDataDefinition(privateUpdateInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(privateUpdateOutputType())
      if(inputError != nil) {
          log.Errorf("Error in ConvertToDataDefinition for PrivateClientImpl.update method's input - %s",
              bindings.VAPIerrorsToError(inputError).Error())
          return nil
      }
      if(outputError != nil) {
          log.Errorf("Error in ConvertToDataDefinition for PrivateClientImpl.update method's output - %s",
	  ...
	  if(errError1 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for PrivateClientImpl.update method's errors.Unauthenticated error - %s",
	  ...
	  if(errError2 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for PrivateClientImpl.update method's errors.InvalidRequest error - %s",
	  ...
	  if(errError3 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for PrivateClientImpl.update method's errors.Unauthorized error - %s",
...
func privateUpdateInputType() bindings.StructType {
...
func privateUpdateOutputType() bindings.BindingType {
    return bindings.NewReferenceType(model.TaskBindingType)
}

func privateUpdateRestMetadata() protocol.OperationRestMetadata {
    ...
    return protocol.NewOperationRestMetadata(
      ...
      "",
      "PUT",
      "/vmc/api/orgs/{org}/sddcs/{sddc}/dns/private",
       resultHeaders,
       200,
       errorHeaders,
