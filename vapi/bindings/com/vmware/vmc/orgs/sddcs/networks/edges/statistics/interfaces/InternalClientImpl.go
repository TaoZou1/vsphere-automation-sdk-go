 ...
 *
 * Client stubs for service: Internal
 * Functions that implement the generated InternalClient interface
 */
...
func NewInternalClientImpl(connector client.Connector) *InternalClientImpl {
      interfaceName := "com.vmware.vmc.orgs.sddcs.networks.edges.statistics.interfaces.internal"
      ...
      iIface := InternalClientImpl{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier:interfaceIdentifier, connector: connector}
      iIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
      iIface.methodNameToDefMap["get"] = iIface.getMethodDefinition()
      return &iIface
}

func (iIface *InternalClientImpl) Get(orgParam string, sddcParam string, edgeIdParam string, startTimeParam *int64, endTimeParam *int64) (model.CbmStatistics, error) {
	typeConverter := iIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(iIface.interfaceIdentifier, "get")
	sv := bindings.NewStructValueBuilder(internalGetInputType(), typeConverter)
	sv.AddStructField("Org", orgParam)
	sv.AddStructField("Sddc", sddcParam)
	sv.AddStructField("EdgeId", edgeIdParam)
	sv.AddStructField("StartTime", startTimeParam)
	sv.AddStructField("EndTime", endTimeParam)
    ...
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), internalGetOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(model.CbmStatistics), nil
	} else {
...
func (iIface *InternalClientImpl) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
    methodResult := iIface.connector.GetApiProvider().Invoke(iIface.interfaceName, methodId.Name(), inputDataValue, ctx)
    return methodResult
}

func (iIface *InternalClientImpl) getMethodDefinition() *core.MethodDefinition {
      interfaceIdentifier := core.NewInterfaceIdentifier(iIface.interfaceName)
      typeConverter := iIface.connector.TypeConverter()

      input, inputError := typeConverter.ConvertToDataDefinition(internalGetInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(internalGetOutputType())
      if(inputError != nil) {
          log.Errorf("Error in ConvertToDataDefinition for InternalClientImpl.get method's input - %s",
              bindings.VAPIerrorsToError(inputError).Error())
          return nil
      }
      if(outputError != nil) {
          log.Errorf("Error in ConvertToDataDefinition for InternalClientImpl.get method's output - %s",
	  ...
	  if(errError1 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for InternalClientImpl.get method's errors.InvalidRequest error - %s",
	  ...
	  if(errError2 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for InternalClientImpl.get method's errors.Unauthorized error - %s",
	  ...
	  if(errError3 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for InternalClientImpl.get method's errors.NotFound error - %s",
...
func internalGetInputType() bindings.StructType {
...
func internalGetOutputType() bindings.BindingType {
    return bindings.NewReferenceType(model.CbmStatisticsBindingType)
}

func internalGetRestMetadata() protocol.OperationRestMetadata {
    ...
    return protocol.NewOperationRestMetadata(
      ...
      "",
      "GET",
      "/vmc/api/orgs/{org}/sddcs/{sddc}/networks/4.0/edges/{edgeId}/statistics/interfaces/internal",
       resultHeaders,
       200,
       errorHeaders,
