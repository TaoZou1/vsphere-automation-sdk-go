...
type Status_Health string

const (
    // NONE. 
    //
    //  No status available.
     Status_Health_NONE Status_Health = "NONE"
    // OK. 
    //
    //  Health is normal.
     Status_Health_OK Status_Health = "OK"
    // WARNING 
    //
    //  Health is normal, however there is an issue that requires attention.
     Status_Health_WARNING Status_Health = "WARNING"
    // ERROR 
    //
    //  Not healthy.
     Status_Health_ERROR Status_Health = "ERROR"
)

func (h Status_Health) Status_Health() bool {
    switch h {
        case Status_Health_NONE:
            return true
        case Status_Health_OK:
            return true
        case Status_Health_WARNING:
            return true
...
func StatusServerInfoBindingType() bindings.BindingType {
    fields := make(map[string]bindings.BindingType)
    fieldNameMap := make(map[string]string)
    fields["health"] = bindings.NewEnumType("com.vmware.esx.kms.providers.status.health", reflect.TypeOf(Status_Health(Status_Health_NONE)))
