 ...
 *
 * Client stubs for service: RawConfig
 * Functions that implement the generated RawConfigClient interface
 */
...
func NewRawConfigClientImpl(connector client.Connector) *RawConfigClientImpl {
      interfaceName := "com.vmware.esx.kms.raw_config"
...
func (rIface *RawConfigClientImpl) Get(type_Param *RawConfig_InfoType) (RawConfigInfo, error) {
	...
	if inputError != nil {
        var emptyOutput RawConfigInfo
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := rawConfigGetRestMetadata()
    ...
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), rawConfigGetOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(RawConfigInfo), nil
	} else {
...
func (rIface *RawConfigClientImpl) Set(specParam RawConfigSetSpec) error {
	...
	operationRestMetaData := rawConfigSetRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	rIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= rIface.Invoke(rIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
    if methodResult.IsSuccess() {
