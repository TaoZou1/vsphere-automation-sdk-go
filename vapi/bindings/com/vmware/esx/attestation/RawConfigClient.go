...
// The ``RawConfig`` interface provides methods to get or put the attestation service persistent user configuration. 
//
...
type RawConfigClient interface {
    ...
    // If {\\\\@term.unset} RawConfig_InfoType#RawConfigInfoType_BRIEF will be used.
    // @return The configuration.
    // @throws Error if there is a generic error.
    // @throws InvalidArgument if the info type is invalid.
    // @throws Unauthenticated if the caller is not authenticated.
    // @throws Unauthorized if the caller is not authorized.
    Get(type_Param *RawConfig_InfoType) (RawConfigInfo, error) 
