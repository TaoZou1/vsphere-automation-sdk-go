...
func NewPolicyClientImpl(connector client.Connector) *PolicyClientImpl {
      interfaceName := "com.vmware.appliance.local_accounts.policy"
...
func (pIface *PolicyClientImpl) Set(policyParam PolicyInfo) error {
	typeConverter := pIface.connector.TypeConverter()
...
func (pIface *PolicyClientImpl) setMethodDefinition() *core.MethodDefinition {
	  ...
	  if(errError1 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for PolicyClientImpl.set method's errors.InvalidArgument error - %s",
	        bindings.VAPIerrorsToError(errError1).Error())
        return nil
	  }
	  errorDefinitions = append(errorDefinitions, errDef1.(data.ErrorDefinition))
