...
//
// #. Create a LibraryItemDeploymentTarget to specify the target deployment type and target deployment designation.
// #. Create a LibraryItemResourcePoolDeploymentSpec to specify the parameters for the target deployment.
...
//
// #. Create a LibraryItemDeployableIdentity to specify the source virtual machine or virtual appliance to be used as the OVF template source.
// #. Create a LibraryItemCreateTarget to specify the target library and library item.
// #. Create a LibraryItemCreateSpec to specify the settings for the OVF package to be created.
...
type LibraryItemClient interface {
    ...
    //
    //  This method retrieves information from the descriptor of the OVF package stored in the library item specified by ``ovf_library_item_id``. The information returned by the method can be used to populate the deployment specification (see LibraryItemResourcePoolDeploymentSpec when deploying the OVF package to the deployment target specified by ``target``. 
    //
    // @param ovfLibraryItemIdParam  Identifier of the content library item containing the OVF package to query.
    // The parameter must be an identifier for the resource type: ``com.vmware.content.library.Item``.
    // @param targetParam  Specification of the deployment target.
    // @return Information that can be used to populate the deployment specification (see LibraryItemResourcePoolDeploymentSpec) when deploying the OVF package to the deployment target specified by ``target``.
    // @throws InvalidArgument  if ``target`` contains invalid arguments.
    // @throws NotFound  if the library item specified by ``ovf_library_item_id`` does not exist.
    // @throws NotFound  if any resource specified by a property of the LibraryItemDeploymentTarget class, specified by ``target``, does not exist.
