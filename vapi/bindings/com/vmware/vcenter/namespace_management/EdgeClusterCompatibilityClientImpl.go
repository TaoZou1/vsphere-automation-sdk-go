 ...
 *
 * Client stubs for service: EdgeClusterCompatibility
 * Functions that implement the generated EdgeClusterCompatibilityClient interface
 */
...
func (eIface *EdgeClusterCompatibilityClientImpl) List(clusterParam string, distributedSwitchParam string, filterParam *EdgeClusterCompatibilityFilterSpec) ([]EdgeClusterCompatibilitySummary, error) {
    ...
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), edgeClusterCompatibilityListOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.([]EdgeClusterCompatibilitySummary), nil
	} else {
