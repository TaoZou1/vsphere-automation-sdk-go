 ...
 *
 * Client stubs for service: Administrators
 * Functions that implement the generated AdministratorsClient interface
 */
...
func (aIface *AdministratorsClientImpl) Set(groupNamesParam map[string]bool) error {
	...
	sv.AddStructField("GroupNames", groupNamesParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		return bindings.VAPIerrorsToError(inputError)
	}
...
func (aIface *AdministratorsClientImpl) Get() (map[string]bool, error) {
	...
	var emptyOutput map[string]bool
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), administratorsGetOutputType())
