...
type CommunityClient interface {
    ...
    // @throws Unauthorized If the caller is not authorized.
    // @throws UnverifiedPeer If the SSL certificate of the target node cannot be validated by comparing with the thumbprint provided in CommunityAddSpec#sslThumbprint or if CommunityAddSpec#sslThumbprint is null and CommunityAddSpec#sslVerify is true. The value of the {\\\\@link UnverifiedPeer#data) property will be a class that contains all the properties defined in CommunityCertificateInfo.
    // @throws Error If the system reports an error while responding to the request.
    Add(specParam CommunityAddSpec) error 
    ...
    // @return list of CommunityCheckInfo objects. Contains results aggregated per pair of nodes.
    // @throws Unauthenticated If unable to authenticate to target node.
    // @throws Unauthorized If the caller is not authorized.
    // @throws UnverifiedPeer If the SSL certificate of the target node cannot be validated by comparing with the thumbprint provided in CommunityAddSpec#sslThumbprint or if CommunityAddSpec#sslThumbprint is null and CommunityAddSpec#sslVerify is true. The value of the {\\\\@link UnverifiedPeer#data) property will be a class that contains all the properties defined in CommunityCertificateInfo.
    // @throws Error If the system reports an error while responding to the request.
    Check(specParam *CommunityAddCheckSpec) ([]CommunityCheckInfo, error) 
