...
type DatacenterClient interface {
    ...
    // If null, the behavior is equivalent to a DatacenterFilterSpec with all properties null which means all datacenters match the filter.
    // @return Commonly used information about the datacenters matching the DatacenterFilterSpec.
    // @throws UnableToAllocateResource if more than 1000 datacenters match the DatacenterFilterSpec.
    // @throws ServiceUnavailable if the system is unable to communicate with a service to complete the request.
    // @throws Unauthenticated if the user can not be authenticated.
    // @throws Unauthorized if the user doesn't have the required privileges.
    ...
    // The parameter must be an identifier for the resource type: ``Datacenter``.
    // @return The DatacenterInfo instances that corresponds to the ``datacenter``.
    // @throws Error if the system reports an error while responding to the request.
    // @throws NotFound if there is no datacenter associated with ``datacenter`` in the system.
    // @throws ServiceUnavailable if the system is unable to communicate with a service to complete the request.
    // @throws Unauthenticated if the user can not be authenticated.
