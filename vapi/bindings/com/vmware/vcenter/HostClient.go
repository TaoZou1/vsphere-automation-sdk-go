...
type HostClient interface {
    ...
    // If null, the behavior is equivalent to a HostFilterSpec with all properties null which means all hosts match the filter.
    // @return Commonly used information about the hosts matching the HostFilterSpec.
    // @throws InvalidArgument if the HostFilterSpec#connectionStates property contains a value that is not supported by the server.
    // @throws UnableToAllocateResource if more than 2500 hosts match the HostFilterSpec.
    // @throws ServiceUnavailable if the system is unable to communicate with a service to complete the request.
    // @throws Unauthenticated if the user can not be authenticated.
    // @throws Unauthorized if the user doesn't have the required privileges.
