 ...
 *
 * Client stubs for service: ReplicationStatus
 * Functions that implement the generated ReplicationStatusClient interface
 */
...
func (rIface *ReplicationStatusClientImpl) List(filterParam *ReplicationStatusFilterSpec) ([]ReplicationStatusSummary, error) {
	...
	if inputError != nil {
        var emptyOutput []ReplicationStatusSummary
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := replicationStatusListRestMetadata
    ...
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), replicationStatusListOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.([]ReplicationStatusSummary), nil
	} else {
