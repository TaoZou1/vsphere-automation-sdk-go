...
func (rIface *ResourcePoolClientImpl) Get(resourcePoolParam string) (ResourcePoolInfo, error) {
    ...
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), resourcePoolGetOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(ResourcePoolInfo), nil
	} else {
...
func (rIface *ResourcePoolClientImpl) List(filterParam *ResourcePoolFilterSpec) ([]ResourcePoolSummary, error) {
	...
	var emptyOutput []ResourcePoolSummary
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), resourcePoolListOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.([]ResourcePoolSummary), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), rIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInError)
...
func (rIface *ResourcePoolClientImpl) Create(specParam ResourcePoolCreateSpec) (string, error) {
	...
	var emptyOutput string
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), resourcePoolCreateOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
