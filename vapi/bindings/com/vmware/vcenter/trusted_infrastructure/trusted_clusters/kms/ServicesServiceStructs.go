...
// The ``Summary`` class contains a summary of a ``Services``. This class was added in vSphere API 7.0.
 type ServicesSummary struct {
    ...
    // The group determines reports issued by which ``Services`` instances can this ``Services`` instance accept. This property was added in vSphere API 7.0.
    Group string
...
// The ``CreateSpec`` class contains the data necessary for adding a ``Services`` to the environment. This class was added in vSphere API 7.0.
 type ServicesCreateSpec struct {
    // Source of truth for the configuration of the Key Provider Service. This property was added in vSphere API 7.0.
...
//
    ...
    // The ``SourceType`` enumeration class lists options which source the the Key Provider Service to use for its configuration. This enumeration was added in vSphere API 7.0.
    //
    ...
    const (
        // The Key Provider Service will be configured based on an ID of an specific Key Provider Service. This constant field was added in vSphere API 7.0.
         ServicesCreateSpec_SourceType_SERVICE ServicesCreateSpec_SourceType = "SERVICE"
        // The Key Provider Service will be configured based on an ID of a whole attestation cluster. This constant field was added in vSphere API 7.0.
...
func ServicesCreateSpecBindingType() bindings.BindingType {
    fields := make(map[string]bindings.BindingType)
    fieldNameMap := make(map[string]string)
    fields["type"] = bindings.NewEnumType("com.vmware.vcenter.trusted_infrastructure.trusted_clusters.kms.services.create_spec.source_type", reflect.TypeOf(ServicesCreateSpec_SourceType(ServicesCreateSpec_SourceType_SERVICE)))
