 ...
 *
 * Client stubs for service: TrustAuthorityClusters
 * Functions that implement the generated TrustAuthorityClustersClient interface
 */
...
func (tIface *TrustAuthorityClustersClientImpl) Get(clusterParam string) (TrustAuthorityClustersInfo, error) {
	...
	var emptyOutput TrustAuthorityClustersInfo
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), trustAuthorityClustersGetOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(TrustAuthorityClustersInfo), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), tIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
...
func (tIface *TrustAuthorityClustersClientImpl) List(specParam *TrustAuthorityClustersFilterSpec) ([]TrustAuthorityClustersSummary, error) {
	...
	var emptyOutput []TrustAuthorityClustersSummary
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), trustAuthorityClustersListOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.([]TrustAuthorityClustersSummary), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), tIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInError)
