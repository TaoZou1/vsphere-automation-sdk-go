 ...
 *
 * Client stubs for service: CurrentPeerCertificates
 * Functions that implement the generated CurrentPeerCertificatesClient interface
 */
...
func (cIface *CurrentPeerCertificatesClientImpl) List(clusterParam string, providerParam string, specParam *CurrentPeerCertificatesFilterSpec) ([]CurrentPeerCertificatesSummary, error) {
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), currentPeerCertificatesListOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.([]CurrentPeerCertificatesSummary), nil
	} else {
