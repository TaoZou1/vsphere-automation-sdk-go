...
//
// * Device_Type#DeviceType_CDROM: Boot from a virtual CD-ROM drive; the device instance(s) will be chosen by the BIOS subsystem.
// * Device_Type#DeviceType_FLOPPY: Boot from a virtual floppy drive; the device instance(s) will be chosen by the BIOS subsystem.
// * Device_Type#DeviceType_DISK: Boot from a virtual disk device; the device instance is specified explicitly in DeviceEntry#disks list, and multiple instances may be specified in the list.
// * Device_Type#DeviceType_ETHERNET: Boot from a virtual Ethernet adapter; the device instance is specified explicitly as DeviceEntry#nic, and multiple adapters may be specified in the boot order list.
type DeviceClient interface {
    ...
    // @throws InvalidArgument if a any of the CDROM, DISK, ETHERNET, FLOPPY values appears in more than one ``Entry`` with the exception of Device_Type#DeviceType_ETHERNET, which may appear multiple times if the virtual machine has been configured with multiple Ethernet adapters.
