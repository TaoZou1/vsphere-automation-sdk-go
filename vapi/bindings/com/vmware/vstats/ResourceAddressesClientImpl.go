...
func (rIface *ResourceAddressesClientImpl) List(filterParam *ResourceAddressesFilterSpec) (ResourceAddressesListResult, error) {
    ...
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), resourceAddressesListOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(ResourceAddressesListResult), nil
	} else {
...
func (rIface *ResourceAddressesClientImpl) Get(idParam string) (ResourceAddressesInfo, error) {
	...
	var emptyOutput ResourceAddressesInfo
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), resourceAddressesGetOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(ResourceAddressesInfo), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), rIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInError)
