 ...
 *
 * Client stubs for service: Data
 * Functions that implement the generated DataClient interface
 */
...
func NewDataClientImpl(connector client.Connector) *DataClientImpl {
	...
	methodIdentifiers := []core.MethodIdentifier{
		core.NewMethodIdentifier(interfaceIdentifier, "queryDataPoints"),
	}
...
func (dIface *DataClientImpl) QueryDataPoints(filterParam *DataFilterSpec) (DataDataPointsResult, error) {
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), dataQueryDataPointsOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(DataDataPointsResult), nil
	} else {
...
func (dIface *DataClientImpl) queryDataPointsMethodDefinition() *core.MethodDefinition {
	...
	methodIdentifier := core.NewMethodIdentifier(interfaceIdentifier, "queryDataPoints")
