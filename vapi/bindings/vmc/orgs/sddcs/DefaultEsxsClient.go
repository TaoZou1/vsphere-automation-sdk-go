...
func NewEsxsClientImpl(connector client.Connector) *EsxsClientImpl {
	...
	eIface := EsxsClientImpl{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (eIface *EsxsClientImpl) createMethodDefinition() *core.MethodDefinition {
	...
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for EsxsClientImpl.create method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for EsxsClientImpl.create method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for EsxsClientImpl.create method's errors.Unauthenticated error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for EsxsClientImpl.create method's errors.InvalidRequest error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for EsxsClientImpl.create method's errors.Unauthorized error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for EsxsClientImpl.create method's errors.NotFound error - %s",
