...
func NewServiceClientImpl(connector client.Connector) *ServiceClientImpl {
      interfaceName := "com.vmware.vapi.metadata.routing.service"
...
func (sIface *ServiceClientImpl) Get(serviceIdParam string) (routing.ServiceInfo, error) {
	typeConverter := sIface.connector.TypeConverter()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
        var emptyOutput routing.ServiceInfo
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
    ...
    if methodResult.IsSuccess() {
	    ...
	    return output.(routing.ServiceInfo), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInError)
