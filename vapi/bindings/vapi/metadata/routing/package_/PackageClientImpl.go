...
func (pIface *PackageClientImpl) Get(packageIdParam string) (routing.PackageInfo, error) {
	typeConverter := pIface.connector.TypeConverter()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
        var emptyOutput routing.PackageInfo
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
    ...
    if methodResult.IsSuccess() {
	    ...
	    return output.(routing.PackageInfo), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), pIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInError)
