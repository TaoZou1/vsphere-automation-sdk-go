...
/*
 * Client stubs for service: ImportFlag
 * Functions that implement the generated ImportFlagClient interface
 */
...
func (iIface *ImportFlagClientImpl) List(rpParam string) ([]ImportFlagInfo, error) {
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), importFlagListOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.([]ImportFlagInfo), nil
	} else {
