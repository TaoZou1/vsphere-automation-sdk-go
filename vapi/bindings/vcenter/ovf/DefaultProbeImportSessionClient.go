...
/*
 * Client stubs for service: ProbeImportSession
 * Functions that implement the generated ProbeImportSessionClient interface
 */
...
func NewDefaultProbeImportSessionClient(connector client.Connector) *DefaultProbeImportSessionClient {
	...
	methodIdentifiers := []core.MethodIdentifier{
		core.NewMethodIdentifier(interfaceIdentifier, "createProbeImportSession"),
	...
	pIface.methodNameToDefMap["try_instantiate"] = pIface.tryInstantiateMethodDefinition()
...
func (pIface *DefaultProbeImportSessionClient) CreateProbeImportSession(clientTokenParam *string, createSpecParam *data.StructValue) (string, error) {
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), probeImportSessionCreateProbeImportSessionOutputType())
...
func (pIface *DefaultProbeImportSessionClient) Get(idParam string) (ProbeImportSessionInfo, error) {
	...
	var emptyOutput ProbeImportSessionInfo
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), probeImportSessionGetOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(ProbeImportSessionInfo), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), pIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInError)
...
func (pIface *DefaultProbeImportSessionClient) TryInstantiate(idParam string, instantiationParametersParam []*data.StructValue) (ImportSessionOvfValidationResult, error) {
	...
	var emptyOutput ImportSessionOvfValidationResult
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), probeImportSessionTryInstantiateOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(ImportSessionOvfValidationResult), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), pIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInError)
...
func (pIface *DefaultProbeImportSessionClient) createProbeImportSessionMethodDefinition() *core.MethodDefinition {
	...
	methodIdentifier := core.NewMethodIdentifier(interfaceIdentifier, "createProbeImportSession")
