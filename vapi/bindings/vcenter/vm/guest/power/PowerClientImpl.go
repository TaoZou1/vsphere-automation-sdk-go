...
func NewPowerClientImpl(connector client.Connector) *PowerClientImpl {
      ...
      methodIdentifiers := []core.MethodIdentifier{
          ...
          core.NewMethodIdentifier(interfaceIdentifier, "shutdown"),
          core.NewMethodIdentifier(interfaceIdentifier, "reboot"),
          core.NewMethodIdentifier(interfaceIdentifier, "standby"),
      }
      ...
      pIface.methodNameToDefMap["reboot"] = pIface.rebootMethodDefinition()
      pIface.methodNameToDefMap["standby"] = pIface.standbyMethodDefinition()
      return &pIface
}
...
func (pIface *PowerClientImpl) Shutdown(vmParam string) error {
	...
	sv := bindings.NewStructValueBuilder(shutdownInputType(), typeConverter)
	sv.AddStructField("Vm", vmParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		return bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := shutdownRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	pIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= pIface.Invoke(pIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
    if methodResult.IsSuccess() {
...
func (pIface *PowerClientImpl) Standby(vmParam string) error {
	...
	sv := bindings.NewStructValueBuilder(standbyInputType(), typeConverter)
	sv.AddStructField("Vm", vmParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		return bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := standbyRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	pIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= pIface.Invoke(pIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
    if methodResult.IsSuccess() {
...
func (pIface *PowerClientImpl) shutdownMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(shutdownInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(shutdownOutputType())
      if(inputError != nil) {
      ...
      methodIdentifier := core.NewMethodIdentifier(interfaceIdentifier, "shutdown")
      errorDefinitions := make([]data.ErrorDefinition,0)
      pIface.errorBindingMap[errors.Error{}.Error()] = errors.ErrorBindingType()
	  errDef1, errError1 := typeConverter.ConvertToDataDefinition(errors.ErrorBindingType())
	  if(errError1 != nil) {
...
func (pIface *PowerClientImpl) standbyMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(standbyInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(standbyOutputType())
      if(inputError != nil) {
      ...
      methodIdentifier := core.NewMethodIdentifier(interfaceIdentifier, "standby")
      errorDefinitions := make([]data.ErrorDefinition,0)
      pIface.errorBindingMap[errors.Error{}.Error()] = errors.ErrorBindingType()
	  errDef1, errError1 := typeConverter.ConvertToDataDefinition(errors.ErrorBindingType())
	  if(errError1 != nil) {
