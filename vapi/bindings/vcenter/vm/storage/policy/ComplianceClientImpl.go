...
func (cIface *ComplianceClientImpl) Get(vmParam string) (*ComplianceInfo, error) {
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), complianceGetOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(*ComplianceInfo), nil
	} else {
...
func (cIface *ComplianceClientImpl) Check(vmParam string, checkSpecParam *ComplianceCheckSpec) (*ComplianceInfo, error) {
	...
	sv.AddStructField("Vm", vmParam)
	sv.AddStructField("CheckSpec", checkSpecParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput *ComplianceInfo
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	...
	var emptyOutput *ComplianceInfo
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), complianceCheckOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(*ComplianceInfo), nil
