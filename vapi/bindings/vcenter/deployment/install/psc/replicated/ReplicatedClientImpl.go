...
type ReplicatedClientImpl struct{
...
func NewReplicatedClientImpl(connector client.Connector) *ReplicatedClientImpl {
      ...
      rIface := ReplicatedClientImpl{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier:interfaceIdentifier, connector: connector}
...
func (rIface *ReplicatedClientImpl) Check(specParam deployment.ReplicatedPscSpec) (deployment.CheckInfo, error) {
...
func (rIface *ReplicatedClientImpl) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
    methodResult := rIface.connector.GetApiProvider().Invoke(rIface.interfaceName, methodId.Name(), inputDataValue, ctx)
    return methodResult
}

func (rIface *ReplicatedClientImpl) checkMethodDefinition() *core.MethodDefinition {
      ...
      if(inputError != nil) {
          log.Errorf("Error in ConvertToDataDefinition for ReplicatedClientImpl.check method's input - %s",
              bindings.VAPIerrorsToError(inputError).Error())
          return nil
      }
      if(outputError != nil) {
          log.Errorf("Error in ConvertToDataDefinition for ReplicatedClientImpl.check method's output - %s",
	  ...
	  if(errError1 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for ReplicatedClientImpl.check method's errors.Unauthenticated error - %s",
	  ...
	  if(errError2 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for ReplicatedClientImpl.check method's errors.InvalidArgument error - %s",
	  ...
	  if(errError3 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for ReplicatedClientImpl.check method's errors.NotAllowedInCurrentState error - %s",
