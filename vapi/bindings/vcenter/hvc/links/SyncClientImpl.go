 ...
 *
 * Client stubs for service: Sync
 * Functions that implement the generated SyncClient interface
 */
...
func NewSyncClientImpl(connector client.Connector) *SyncClientImpl {
	...
	methodIdentifiers := []core.MethodIdentifier{
		...
		core.NewMethodIdentifier(interfaceIdentifier, "remediate"),
	}
...
func (sIface *SyncClientImpl) Reset(linkParam string) error {
	...
	operationRestMetaData := syncResetRestMetadata()
...
func (sIface *SyncClientImpl) Remediate(linkParam string, credentialsParam *SyncCredentials) (SyncRemediationStatus, error) {
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput SyncRemediationStatus
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := syncRemediateRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.Invoke(sIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
	var emptyOutput SyncRemediationStatus
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), syncRemediateOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(SyncRemediationStatus), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
...
func (sIface *SyncClientImpl) remediateMethodDefinition() *core.MethodDefinition {
	...
	methodIdentifier := core.NewMethodIdentifier(interfaceIdentifier, "remediate")
	errorDefinitions := make([]data.ErrorDefinition, 0)
	sIface.errorBindingMap[errors.Error{}.Error()] = errors.ErrorBindingType()
	errDef1, errError1 := typeConverter.ConvertToDataDefinition(errors.ErrorBindingType())
	if errError1 != nil {
