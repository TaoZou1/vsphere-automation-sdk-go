...
func (cIface *ClusterClientImpl) List(filterParam *ClusterFilterSpec) ([]ClusterSummary, error) {
	...
	if inputError != nil {
		var emptyOutput []ClusterSummary
	...
	if methodResult.IsSuccess() {
		...
		return output.([]ClusterSummary), nil
	} else {
...
func (cIface *ClusterClientImpl) Create(specParam ClusterCreateSpec) (string, error) {
	...
	var emptyOutput string
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), clusterCreateOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
