...
type DefaultFeatureStateClient struct {
...
func NewDefaultFeatureStateClient(connector client.Connector) *DefaultFeatureStateClient {
	...
	fIface := DefaultFeatureStateClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	fIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	fIface.methodNameToDefMap["get"] = fIface.getMethodDefinition()
	return &fIface
}

func (fIface *DefaultFeatureStateClient) Get(featuresParam []string) (map[string]FeatureStateStatus, error) {
...
func (fIface *DefaultFeatureStateClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := fIface.connector.GetApiProvider().Invoke(fIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (fIface *DefaultFeatureStateClient) getMethodDefinition() *core.MethodDefinition {
	...
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFeatureStateClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFeatureStateClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFeatureStateClient.get method's errors.Error error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFeatureStateClient.get method's errors.NotFound error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFeatureStateClient.get method's errors.Unauthenticated error - %s",
