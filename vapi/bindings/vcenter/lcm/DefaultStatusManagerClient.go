...
/*
 * Client stubs for service: StatusManager
 * Functions that implement the generated StatusManagerClient interface
 */
...
func (sIface *StatusManagerClientImpl) Check() (std.LocalizableMessage, error) {
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), statusManagerCheckOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(std.LocalizableMessage), nil
	} else {
