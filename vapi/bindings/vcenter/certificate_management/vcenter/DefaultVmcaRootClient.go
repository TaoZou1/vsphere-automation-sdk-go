...
type DefaultVmcaRootClient struct {
...
func NewDefaultVmcaRootClient(connector client.Connector) *DefaultVmcaRootClient {
	...
	vIface := DefaultVmcaRootClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (vIface *DefaultVmcaRootClient) Create(specParam *VmcaRootCreateSpec) error {
...
func (vIface *DefaultVmcaRootClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := vIface.connector.GetApiProvider().Invoke(vIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (vIface *DefaultVmcaRootClient) createMethodDefinition() *core.MethodDefinition {
	...
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVmcaRootClient.create method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVmcaRootClient.create method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVmcaRootClient.create method's errors.Error error - %s",
