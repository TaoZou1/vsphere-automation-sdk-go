...
/*
 * Client stubs for service: ClusterCompatibility
 * Functions that implement the generated ClusterCompatibilityClient interface
 */
...
func (cIface *DefaultClusterCompatibilityClient) List(filterParam *ClusterCompatibilityFilterSpec) ([]ClusterCompatibilitySummary, error) {
	...
	if inputError != nil {
		var emptyOutput []ClusterCompatibilitySummary
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := clusterCompatibilityListRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := cIface.Invoke(cIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
	var emptyOutput []ClusterCompatibilitySummary
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), clusterCompatibilityListOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.([]ClusterCompatibilitySummary), nil
	} else {
