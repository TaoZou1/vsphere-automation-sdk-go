...
/*
 * Client stubs for service: ClusterAvailableVersions
 * Functions that implement the generated ClusterAvailableVersionsClient interface
 */
...
func (cIface *ClusterAvailableVersionsClientImpl) List() ([]ClusterAvailableVersionsSummary, error) {
	...
	if inputError != nil {
		var emptyOutput []ClusterAvailableVersionsSummary
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := clusterAvailableVersionsListRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := cIface.Invoke(cIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
	var emptyOutput []ClusterAvailableVersionsSummary
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), clusterAvailableVersionsListOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.([]ClusterAvailableVersionsSummary), nil
	} else {
