 ...
 *
 * Client stubs for service: TimeSeries
 * Functions that implement the generated TimeSeriesClient interface
 */
...
func (tIface *TimeSeriesClientImpl) Get(specParam TimeSeriesSpec) ([]TimeSeriesTimeSeries, error) {
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), timeSeriesGetOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.([]TimeSeriesTimeSeries), nil
	} else {
