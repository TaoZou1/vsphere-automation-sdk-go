...
type CategoryClient interface {
    ...
    // @throws AlreadyExists  if the CreateSpec#name provided in the ``create_spec`` is the name of an already existing category.
    ...
    // @return The tagging.CategoryModel that corresponds to ``category_id``.
    ...
    Get(categoryIdParam string) (tagging.CategoryModel, error) 
    ...
    // @throws AlreadyExists  if the UpdateSpec#name provided in the ``update_spec`` is the name of an already existing category.
    ...
    Update(categoryIdParam string, updateSpecParam UpdateSpec) error 
    ...
    // Enumerates all categories for which the ``used_by_entity`` is part of the tagging.CategoryModel#usedBy subscribers map with bool value. To invoke this method, you need the read privilege on the individual categories.
    //
    ...
    // Adds the ``used_by_entity`` to the tagging.CategoryModel#usedBy subscribers map with bool value for the specified category. If the ``used_by_entity`` is already in the map with bool value, then this becomes an idempotent no-op. To invoke this method, you need the modify tagging.CategoryModel#usedBy privilege on the category.
    //
    // @param categoryIdParam  The identifier of the input category.
    // The parameter must be an identifier for the resource type: ``com.vmware.cis.tagging.Category``.
    // @param usedByEntityParam  The name of the user to be added to the tagging.CategoryModel#usedBy map with bool value.
    // @throws NotFound  if the category for the given ``category_id`` does not exist in the system.
    // @throws Unauthorized  if you do not have the privilege to add an entity to the tagging.CategoryModel#usedBy field.
    AddToUsedBy(categoryIdParam string, usedByEntityParam string) error 


    // Removes the ``used_by_entity`` from the tagging.CategoryModel#usedBy subscribers map with bool value. If the ``used_by_entity`` is not using this category, then this becomes a no-op. To invoke this method, you need the modify tagging.CategoryModel#usedBy privilege on the category.
    //
    // @param categoryIdParam  The identifier of the input category.
    // The parameter must be an identifier for the resource type: ``com.vmware.cis.tagging.Category``.
    // @param usedByEntityParam  The name of the user to be removed from the tagging.CategoryModel#usedBy map with bool value.
    // @throws NotFound  if the category for the given ``category_id`` does not exist in the system.
    // @throws Unauthorized  if you do not have the privilege to remove an entity from the tagging.CategoryModel#usedBy field.
