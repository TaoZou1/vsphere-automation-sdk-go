...
/*
 * Client stubs for service: Remote
 * Functions that implement the generated RemoteClient interface
 */
...
func (rIface *RemoteClientImpl) List(providerParam string, specParam *RemoteFilterSpec) ([]RemoteSummary, error) {
	...
	if inputError != nil {
		var emptyOutput []RemoteSummary
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := remoteListRestMetadata()
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), remoteListOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.([]RemoteSummary), nil
	} else {
