...
/*
 * Client stubs for service: ResourceAddressSchemas
 * Functions that implement the generated ResourceAddressSchemasClient interface
 */
...
func (rIface *DefaultResourceAddressSchemasClient) Get(idParam string) (ResourceAddressSchemasInfo, error) {
	...
	if inputError != nil {
		var emptyOutput ResourceAddressSchemasInfo
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := resourceAddressSchemasGetRestMetadata()
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), resourceAddressSchemasGetOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(ResourceAddressSchemasInfo), nil
	} else {
