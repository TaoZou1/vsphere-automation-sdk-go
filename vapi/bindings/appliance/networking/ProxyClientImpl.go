 ...
 *
 * Client stubs for service: Proxy
 * Functions that implement the generated ProxyClient interface
 */
...
func (pIface *ProxyClientImpl) Test(hostParam string, protocolParam string, configParam ProxyConfig) (ProxyTestResult, error) {
	...
	sv.AddStructField("Protocol", protocolParam)
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), proxyTestOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(ProxyTestResult), nil
	} else {
...
func (pIface *ProxyClientImpl) Get(protocolParam string) (ProxyConfig, error) {
	...
	var emptyOutput ProxyConfig
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), proxyGetOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(ProxyConfig), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), pIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInError)
