...
/*
 * Client stubs for service: Time
 * Functions that implement the generated TimeClient interface
 */
...
func (tIface *DefaultTimeClient) Get() (TimeSystemTimeStruct, error) {
	...
	if inputError != nil {
		var emptyOutput TimeSystemTimeStruct
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := timeGetRestMetadata()
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), timeGetOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(TimeSystemTimeStruct), nil
	} else {
