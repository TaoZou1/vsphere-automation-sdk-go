...
/*
 * Client stubs for service: Timezone
 * Functions that implement the generated TimezoneClient interface
 */
...
func (tIface *DefaultTimezoneClient) Set(nameParam string) error {
	...
	operationRestMetaData := timezoneSetRestMetadata()
...
func (tIface *DefaultTimezoneClient) Get() (string, error) {
	...
	operationRestMetaData := timezoneGetRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	tIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := tIface.Invoke(tIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
	var emptyOutput string
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), timezoneGetOutputType())
