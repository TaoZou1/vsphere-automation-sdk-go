 ...
 *
 * Client stubs for service: Archive
 * Functions that implement the generated ArchiveClient interface
 */

package system_name

import (
	"gitlab.eng.vmware.com/golangsdk/vsphere-automation-sdk-go/vapi/bindings/appliance/recovery/backup"
...
func (aIface *ArchiveClientImpl) Get(specParam backup.LocationSpec, systemNameParam string, archiveParam string) (ArchiveInfo, error) {
	...
	sv.AddStructField("SystemName", systemNameParam)
	sv.AddStructField("Archive", archiveParam)
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), archiveGetOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(ArchiveInfo), nil
	} else {
...
func (aIface *ArchiveClientImpl) List(locSpecParam backup.LocationSpec, systemNameParam string, filterSpecParam ArchiveFilterSpec) ([]ArchiveSummary, error) {
	...
	var emptyOutput []ArchiveSummary
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), archiveListOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.([]ArchiveSummary), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), aIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInError)
