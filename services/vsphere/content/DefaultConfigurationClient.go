...
func NewDefaultConfigurationClient(connector client.Connector) *DefaultConfigurationClient {
	interfaceName := "com.vmware.content.configuration"
...
func (cIface *DefaultConfigurationClient) Update(modelParam ConfigurationModel) error {
	...
	sv.AddStructField("Model", modelParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		return bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := configurationUpdateRestMetadata()
...
func (cIface *DefaultConfigurationClient) Get() (ConfigurationModel, error) {
	typeConverter := cIface.connector.TypeConverter()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput ConfigurationModel
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := configurationGetRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	connectionMetadata["isStreamingResponse"] = false
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	executionContext := cIface.connector.NewExecutionContext()
	...
	var emptyOutput ConfigurationModel
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), configurationGetOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(ConfigurationModel), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), cIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
