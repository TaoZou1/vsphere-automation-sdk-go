...
func NewDefaultFileClient(connector client.Connector) *DefaultFileClient {
	interfaceName := "com.vmware.content.library.item.downloadsession.file"
	interfaceIdentifier := core.NewInterfaceIdentifier(interfaceName)
	methodIdentifiers := []core.MethodIdentifier{
		core.NewMethodIdentifier(interfaceIdentifier, "list"),
		core.NewMethodIdentifier(interfaceIdentifier, "prepare"),
	...
	fIface.methodNameToDefMap["prepare"] = fIface.prepareMethodDefinition()
	fIface.methodNameToDefMap["get"] = fIface.getMethodDefinition()
	return &fIface
}

func (fIface *DefaultFileClient) List(downloadSessionIdParam string) ([]FileInfo, error) {
...
func (fIface *DefaultFileClient) Prepare(downloadSessionIdParam string, fileNameParam string, endpointTypeParam *FileEndpointType) (FileInfo, error) {
	typeConverter := fIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(fIface.interfaceIdentifier, "prepare")
	sv := bindings.NewStructValueBuilder(filePrepareInputType(), typeConverter)
	sv.AddStructField("DownloadSessionId", downloadSessionIdParam)
	sv.AddStructField("FileName", fileNameParam)
	sv.AddStructField("EndpointType", endpointTypeParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput FileInfo
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := filePrepareRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	connectionMetadata["isStreamingResponse"] = false
	fIface.connector.SetConnectionMetadata(connectionMetadata)
	executionContext := fIface.connector.NewExecutionContext()
	...
	var emptyOutput FileInfo
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), filePrepareOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(FileInfo), nil
...
func (fIface *DefaultFileClient) Get(downloadSessionIdParam string, fileNameParam string) (FileInfo, error) {
	typeConverter := fIface.connector.TypeConverter()
...
func (fIface *DefaultFileClient) prepareMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(fIface.interfaceName)
	typeConverter := fIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(filePrepareInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(filePrepareOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFileClient.prepare method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFileClient.prepare method's output - %s",
			bindings.VAPIerrorsToError(outputError).Error())
		return nil
	}
	methodIdentifier := core.NewMethodIdentifier(interfaceIdentifier, "prepare")
	errorDefinitions := make([]data.ErrorDefinition, 0)
	fIface.errorBindingMap[errors.NotFound{}.Error()] = errors.NotFoundBindingType()
	errDef1, errError1 := typeConverter.ConvertToDataDefinition(errors.NotFoundBindingType())
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFileClient.prepare method's errors.NotFound error - %s",
			bindings.VAPIerrorsToError(errError1).Error())
		return nil
	}
	errorDefinitions = append(errorDefinitions, errDef1.(data.ErrorDefinition))
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFileClient.prepare method's errors.InvalidArgument error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFileClient.prepare method's errors.Unauthorized error - %s",
