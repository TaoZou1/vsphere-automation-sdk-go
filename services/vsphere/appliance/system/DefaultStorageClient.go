...
func NewDefaultStorageClient(connector client.Connector) *DefaultStorageClient {
	...
	methodIdentifiers := []core.MethodIdentifier{
		...
		core.NewMethodIdentifier(interfaceIdentifier, "resize"),
		core.NewMethodIdentifier(interfaceIdentifier, "resizeEx"),
	}
...
func (sIface *DefaultStorageClient) List() ([]StorageStorageMapping, error) {
	...
	if inputError != nil {
		var emptyOutput []StorageStorageMapping
	...
	if methodResult.IsSuccess() {
		...
		return output.([]StorageStorageMapping), nil
	} else {
...
func (sIface *DefaultStorageClient) Resize() error {
	...
	operationRestMetaData := storageResizeRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	executionContext := sIface.connector.NewExecutionContext()
...
func (sIface *DefaultStorageClient) ResizeEx() (map[string]StorageStorageChange, error) {
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput map[string]StorageStorageChange
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := storageResizeExRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	executionContext := sIface.connector.NewExecutionContext()
	...
	var emptyOutput map[string]StorageStorageChange
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), storageResizeExOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(map[string]StorageStorageChange), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInError)
...
func (sIface *DefaultStorageClient) resizeMethodDefinition() *core.MethodDefinition {
	...
	methodIdentifier := core.NewMethodIdentifier(interfaceIdentifier, "resize")
	errorDefinitions := make([]data.ErrorDefinition, 0)
	sIface.errorBindingMap[errors.Error{}.Error()] = errors.ErrorBindingType()
	errDef1, errError1 := typeConverter.ConvertToDataDefinition(errors.ErrorBindingType())
	if errError1 != nil {
...
func (sIface *DefaultStorageClient) resizeExMethodDefinition() *core.MethodDefinition {
	...
	methodIdentifier := core.NewMethodIdentifier(interfaceIdentifier, "resizeEx")
	errorDefinitions := make([]data.ErrorDefinition, 0)
	sIface.errorBindingMap[errors.Error{}.Error()] = errors.ErrorBindingType()
	errDef1, errError1 := typeConverter.ConvertToDataDefinition(errors.ErrorBindingType())
	if errError1 != nil {
