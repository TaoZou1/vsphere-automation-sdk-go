...
/*
 * Client stubs for service: NoProxy
 * Functions that implement the generated NoProxyClient interface
 */
...
func (nIface *DefaultNoProxyClient) Set(serversParam []string) error {
	...
	operationRestMetaData := noProxySetRestMetadata()
...
func (nIface *DefaultNoProxyClient) Get() ([]string, error) {
	...
	operationRestMetaData := noProxyGetRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	connectionMetadata["isStreamingResponse"] = false
	nIface.connector.SetConnectionMetadata(connectionMetadata)
	executionContext := nIface.connector.NewExecutionContext()
	...
	var emptyOutput []string
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), noProxyGetOutputType())
