...
func (fIface *DefaultForwardingClient) Test(sendTestMessageParam *bool) ([]ForwardingConnectionStatus, error) {
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), forwardingTestOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.([]ForwardingConnectionStatus), nil
	} else {
...
func (fIface *DefaultForwardingClient) Get() ([]ForwardingConfig, error) {
	...
	if methodResult.IsSuccess() {
		...
		return output.([]ForwardingConfig), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), fIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInError)
