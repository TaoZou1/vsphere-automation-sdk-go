...
func (pIface *DefaultPolicyClient) Set(policyParam PolicyInfo) error {
	typeConverter := pIface.connector.TypeConverter()
...
func (pIface *DefaultPolicyClient) setMethodDefinition() *core.MethodDefinition {
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultPolicyClient.set method's errors.InvalidArgument error - %s",
			bindings.VAPIerrorsToError(errError1).Error())
		return nil
	}
	errorDefinitions = append(errorDefinitions, errDef1.(data.ErrorDefinition))
