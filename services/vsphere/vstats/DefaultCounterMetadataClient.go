...
/*
 * Client stubs for service: CounterMetadata
 * Functions that implement the generated CounterMetadataClient interface
 */
...
func (cIface *DefaultCounterMetadataClient) List(cidParam string, filterParam *CounterMetadataFilterSpec) ([]CounterMetadataInfo, error) {
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), counterMetadataListOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.([]CounterMetadataInfo), nil
	} else {
...
func (cIface *DefaultCounterMetadataClient) GetDefault(cidParam string) ([]CounterMetadataInfo, error) {
	...
	var emptyOutput []CounterMetadataInfo
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), counterMetadataGetDefaultOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.([]CounterMetadataInfo), nil
...
func (cIface *DefaultCounterMetadataClient) Get(cidParam string, midParam string) (CounterMetadataInfo, error) {
	...
	sv.AddStructField("Cid", cidParam)
	sv.AddStructField("Mid", midParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput CounterMetadataInfo
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	...
	var emptyOutput CounterMetadataInfo
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), counterMetadataGetOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(CounterMetadataInfo), nil
