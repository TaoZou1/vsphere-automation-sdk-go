...
/*
 * Client stubs for service: Folder
 * Functions that implement the generated FolderClient interface
 */
...
func (fIface *DefaultFolderClient) List(filterParam *FolderFilterSpec) ([]FolderSummary, error) {
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), folderListOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.([]FolderSummary), nil
	} else {
