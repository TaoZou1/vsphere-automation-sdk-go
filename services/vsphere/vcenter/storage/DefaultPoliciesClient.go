...
func (pIface *DefaultPoliciesClient) CheckCompatibility(policyParam string, datastoresParam map[string]bool) (PoliciesCompatibilityInfo, error) {
	...
	var emptyOutput PoliciesCompatibilityInfo
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), policiesCheckCompatibilityOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(PoliciesCompatibilityInfo), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), pIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInError)
