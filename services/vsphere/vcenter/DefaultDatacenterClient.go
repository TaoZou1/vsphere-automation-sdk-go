...
/*
 * Client stubs for service: Datacenter
 * Functions that implement the generated DatacenterClient interface
 */
...
func (dIface *DefaultDatacenterClient) Create(specParam DatacenterCreateSpec) (string, error) {
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), datacenterCreateOutputType())
...
func (dIface *DefaultDatacenterClient) Delete(datacenterParam string, forceParam *bool) error {
	...
	sv.AddStructField("Datacenter", datacenterParam)
...
func (dIface *DefaultDatacenterClient) List(filterParam *DatacenterFilterSpec) ([]DatacenterSummary, error) {
	...
	var emptyOutput []DatacenterSummary
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), datacenterListOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.([]DatacenterSummary), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), dIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInError)
...
func (dIface *DefaultDatacenterClient) Get(datacenterParam string) (DatacenterInfo, error) {
	...
	var emptyOutput DatacenterInfo
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), datacenterGetOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(DatacenterInfo), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), dIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInError)
