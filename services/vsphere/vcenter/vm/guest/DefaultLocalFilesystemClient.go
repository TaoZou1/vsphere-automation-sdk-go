...
/*
 * Client stubs for service: LocalFilesystem
 * Functions that implement the generated LocalFilesystemClient interface
 */
...
func (lIface *DefaultLocalFilesystemClient) Get(vmParam string) (map[string]LocalFilesystemInfo, error) {
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), localFilesystemGetOutputType())
		if errorInOutput != nil {
			return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
		return output.(map[string]LocalFilesystemInfo), nil
	} else {
