...
func NewNsxPartialPatchConfigClient(connector client.Connector) *nsxPartialPatchConfigClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.system_config.nsx_partial_patch_config")
...
func (nIface *nsxPartialPatchConfigClient) Get() (model.PartialPatchConfig, error) {
	...
	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.system_config.nsx_partial_patch_config", "get", inputDataValue, executionContext)
	var emptyOutput model.PartialPatchConfig
...
func (nIface *nsxPartialPatchConfigClient) Patch(partialPatchConfigParam model.PartialPatchConfig) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	nIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.system_config.nsx_partial_patch_config", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), nIface.GetErrorBindingType(methodResult.Error().Name()))
