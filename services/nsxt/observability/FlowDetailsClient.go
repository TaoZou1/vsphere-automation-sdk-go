...
func NewFlowDetailsClient(connector client.Connector) *flowDetailsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.observability.flow_details")
...
func (fIface *flowDetailsClient) Get(sitePathParam *string) (model.MonitoringInfo, error) {
	...
	methodResult := fIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.observability.flow_details", "get", inputDataValue, executionContext)
	var emptyOutput model.MonitoringInfo
