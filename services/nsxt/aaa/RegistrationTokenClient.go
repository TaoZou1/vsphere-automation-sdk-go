...
func NewRegistrationTokenClient(connector client.Connector) *registrationTokenClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.aaa.registration_token")
...
func (rIface *registrationTokenClient) Create() (model.RegistrationToken, error) {
	...
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.aaa.registration_token", "create", inputDataValue, executionContext)
	var emptyOutput model.RegistrationToken
...
func (rIface *registrationTokenClient) Delete(tokenParam string) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	rIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.aaa.registration_token", "delete", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), rIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (rIface *registrationTokenClient) Get(tokenParam string) (model.RegistrationToken, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	rIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.aaa.registration_token", "get", inputDataValue, executionContext)
	var emptyOutput model.RegistrationToken
	if methodResult.IsSuccess() {
