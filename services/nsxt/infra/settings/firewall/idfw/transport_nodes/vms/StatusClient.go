...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.settings.firewall.idfw.transport_nodes.vms.status")
...
func (sIface *statusClient) List(transportNodeIdParam string, enforcementPointPathParam *string) (model.IdfwVirtualMachineStatusListResult, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.settings.firewall.idfw.transport_nodes.vms.status", "list", inputDataValue, executionContext)
	var emptyOutput model.IdfwVirtualMachineStatusListResult
