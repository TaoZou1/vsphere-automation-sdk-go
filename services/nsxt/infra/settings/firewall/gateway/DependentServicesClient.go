...
func NewDependentServicesClient(connector client.Connector) *dependentServicesClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.settings.firewall.gateway.dependent_services")
...
func (dIface *dependentServicesClient) Get() (model.DependentServices, error) {
	...
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.settings.firewall.gateway.dependent_services", "get", inputDataValue, executionContext)
	var emptyOutput model.DependentServices
