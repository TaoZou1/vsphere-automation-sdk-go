...
type StatusClient interface {
	...
	//
	// @param applianceIdParam ID of appliance node (required)
	...
	Get(applianceIdParam string) (model.ApplianceLatencyData, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List(cursorParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.ApplianceLatencyListResult, error)
}
...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sha.appliances.latency.status")
...
func (sIface *statusClient) Get(applianceIdParam string) (model.ApplianceLatencyData, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.sha.appliances.latency.status", "get", inputDataValue, executionContext)
	var emptyOutput model.ApplianceLatencyData
...
func (sIface *statusClient) List(cursorParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.ApplianceLatencyListResult, error) {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	...
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.sha.appliances.latency.status", "list", inputDataValue, executionContext)
	var emptyOutput model.ApplianceLatencyListResult
	if methodResult.IsSuccess() {
