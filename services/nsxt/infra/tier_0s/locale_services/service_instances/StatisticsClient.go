...
func NewStatisticsClient(connector client.Connector) *statisticsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.tier_0s.locale_services.service_instances.statistics")
...
func (sIface *statisticsClient) Get(tier0IdParam string, localeServiceIdParam string, serviceInstanceIdParam string, enforcementPointPathParam *string) (model.PolicyServiceInstanceStatistics, error) {
	...
	if inputError != nil {
		var emptyOutput model.PolicyServiceInstanceStatistics
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.tier_0s.locale_services.service_instances.statistics", "get", inputDataValue, executionContext)
	var emptyOutput model.PolicyServiceInstanceStatistics
	if methodResult.IsSuccess() {
		...
		return output.(model.PolicyServiceInstanceStatistics), nil
	} else {
