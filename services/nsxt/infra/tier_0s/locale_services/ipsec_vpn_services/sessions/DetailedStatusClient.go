...
type DetailedStatusClient interface {
	...
	//  This API is deprecated. Please use GET /infra/tier-0s/<tier-0-id>/ipsec-vpn-services/<service-id>/ sessions/<session-id>/detailed-status instead.
	//
...
func NewDetailedStatusClient(connector client.Connector) *detailedStatusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.tier_0s.locale_services.ipsec_vpn_services.sessions.detailed_status")
...
func (dIface *detailedStatusClient) Get(tier0IdParam string, localeServiceIdParam string, serviceIdParam string, sessionIdParam string, enforcementPointPathParam *string, sourceParam *string) (model.AggregateIPSecVpnSessionStatus, error) {
	...
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.tier_0s.locale_services.ipsec_vpn_services.sessions.detailed_status", "get", inputDataValue, executionContext)
	var emptyOutput model.AggregateIPSecVpnSessionStatus
