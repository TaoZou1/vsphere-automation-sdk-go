...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sites.napp.deployment.platform.status")
...
func (sIface *statusClient) Get(siteIdParam string) (model.DeploymentProgressStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.sites.napp.deployment.platform.status", "get", inputDataValue, executionContext)
	var emptyOutput model.DeploymentProgressStatus
