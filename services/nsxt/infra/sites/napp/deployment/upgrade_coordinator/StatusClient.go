...
type StatusClient interface {
	...
	Get(siteIdParam string) (model.DeploymentProgressStatus, error)
}
...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sites.napp.deployment.upgrade_coordinator.status")
...
func (sIface *statusClient) Get(siteIdParam string) (model.DeploymentProgressStatus, error) {
	...
	if inputError != nil {
		var emptyOutput model.DeploymentProgressStatus
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.sites.napp.deployment.upgrade_coordinator.status", "get", inputDataValue, executionContext)
	var emptyOutput model.DeploymentProgressStatus
	if methodResult.IsSuccess() {
		...
		return output.(model.DeploymentProgressStatus), nil
	} else {
