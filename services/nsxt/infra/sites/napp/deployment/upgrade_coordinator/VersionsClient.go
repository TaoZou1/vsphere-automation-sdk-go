...
func NewVersionsClient(connector client.Connector) *versionsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sites.napp.deployment.upgrade_coordinator.versions")
...
func (vIface *versionsClient) Get(siteIdParam string) (model.DeploymentVersions, error) {
	...
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.sites.napp.deployment.upgrade_coordinator.versions", "get", inputDataValue, executionContext)
	var emptyOutput model.DeploymentVersions
