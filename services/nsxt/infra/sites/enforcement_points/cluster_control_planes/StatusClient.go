...
type StatusClient interface {
	...
	Get(siteIdParam string, enforcementpointIdParam string, clusterControlPlaneIdParam string) (model.AntreaClusterInfo, error)
}
...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sites.enforcement_points.cluster_control_planes.status")
...
func (sIface *statusClient) Get(siteIdParam string, enforcementpointIdParam string, clusterControlPlaneIdParam string) (model.AntreaClusterInfo, error) {
	...
	if inputError != nil {
		var emptyOutput model.AntreaClusterInfo
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.sites.enforcement_points.cluster_control_planes.status", "get", inputDataValue, executionContext)
	var emptyOutput model.AntreaClusterInfo
	if methodResult.IsSuccess() {
		...
		return output.(model.AntreaClusterInfo), nil
	} else {
