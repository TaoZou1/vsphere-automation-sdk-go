...
func NewSummaryClient(connector client.Connector) *summaryClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.segments.gateway_interface_statistics.summary")
...
func (sIface *summaryClient) Get(segmentIdParam string, enforcementPointPathParam *string, sourceParam *string) (model.PolicyInterfaceStatisticsSummary, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.segments.gateway_interface_statistics.summary", "get", inputDataValue, executionContext)
	var emptyOutput model.PolicyInterfaceStatisticsSummary
