...
func NewNslookupClient(connector client.Connector) *nslookupClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.tier_1s.dns_forwarder.nslookup")
...
func (nIface *nslookupClient) Get(tier1IdParam string, addressParam *string, enforcementPointPathParam *string) (model.AggregatePolicyDnsAnswer, error) {
	...
	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.tier_1s.dns_forwarder.nslookup", "get", inputDataValue, executionContext)
	var emptyOutput model.AggregatePolicyDnsAnswer
