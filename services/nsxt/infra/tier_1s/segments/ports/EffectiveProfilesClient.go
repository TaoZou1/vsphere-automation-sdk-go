...
type EffectiveProfilesClient interface {
	...
	Get(tier1IdParam string, segmentIdParam string, portIdParam string, enforcementPointPathParam *string) (model.EffectiveProfilesResponse, error)
}
...
func NewEffectiveProfilesClient(connector client.Connector) *effectiveProfilesClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.tier_1s.segments.ports.effective_profiles")
...
func (eIface *effectiveProfilesClient) Get(tier1IdParam string, segmentIdParam string, portIdParam string, enforcementPointPathParam *string) (model.EffectiveProfilesResponse, error) {
	...
	methodResult := eIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.tier_1s.segments.ports.effective_profiles", "get", inputDataValue, executionContext)
	var emptyOutput model.EffectiveProfilesResponse
