...
func NewStateClient(connector client.Connector) *stateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.tier_1s.segments.dhcp_static_bindings.state")
...
func (sIface *stateClient) Get(tier1IdParam string, segmentIdParam string, bindingIdParam string) (model.DhcpStaticBindingState, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.tier_1s.segments.dhcp_static_bindings.state", "get", inputDataValue, executionContext)
	var emptyOutput model.DhcpStaticBindingState
