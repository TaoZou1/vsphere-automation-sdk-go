...
type RemoteMacClient interface {
	...
	//  This API is deprecated. Please use GET /infra/tier-1s/<tier-1-id>/l2vpn-services/<service-id>/sessions/<session-id>/remote-mac instead.
	//
...
func NewRemoteMacClient(connector client.Connector) *remoteMacClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.tier_1s.locale_services.l2vpn_services.sessions.remote_mac")
...
func (rIface *remoteMacClient) Get(tier1IdParam string, localeServiceIdParam string, serviceIdParam string, sessionIdParam string, enforcementPointPathParam *string, segmentPathParam *string) (model.AggregateL2VpnSessionRemoteMac, error) {
	...
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.tier_1s.locale_services.l2vpn_services.sessions.remote_mac", "get", inputDataValue, executionContext)
	var emptyOutput model.AggregateL2VpnSessionRemoteMac
