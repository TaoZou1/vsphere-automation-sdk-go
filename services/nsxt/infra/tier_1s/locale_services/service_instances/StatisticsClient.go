...
func NewStatisticsClient(connector client.Connector) *statisticsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.tier_1s.locale_services.service_instances.statistics")
...
func (sIface *statisticsClient) Get(tier1IdParam string, localeServiceIdParam string, serviceInstanceIdParam string, enforcementPointPathParam *string) (model.PolicyServiceInstanceStatistics, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.tier_1s.locale_services.service_instances.statistics", "get", inputDataValue, executionContext)
	var emptyOutput model.PolicyServiceInstanceStatistics
