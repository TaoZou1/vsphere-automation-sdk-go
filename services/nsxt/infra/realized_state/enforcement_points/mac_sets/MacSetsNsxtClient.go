...
func NewMacSetsNsxtClient(connector client.Connector) *macSetsNsxtClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.realized_state.enforcement_points.mac_sets.mac_sets_nsxt")
...
func (mIface *macSetsNsxtClient) GetErrorBindingType(errorName string) bindings.BindingType {
...
func (mIface *macSetsNsxtClient) Get(enforcementPointNameParam string, macSetNameParam string) (model.GenericPolicyRealizedResource, error) {
	typeConverter := mIface.connector.TypeConverter()
	executionContext := mIface.connector.NewExecutionContext()
	sv := bindings.NewStructValueBuilder(macSetsNsxtGetInputType(), typeConverter)
...
func (mIface *macSetsNsxtClient) List(enforcementPointNameParam string, cursorParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.GenericPolicyRealizedResourceListResult, error) {
	typeConverter := mIface.connector.TypeConverter()
	executionContext := mIface.connector.NewExecutionContext()
	sv := bindings.NewStructValueBuilder(macSetsNsxtListInputType(), typeConverter)
	sv.AddStructField("EnforcementPointName", enforcementPointNameParam)
