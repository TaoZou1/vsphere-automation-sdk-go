...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.metadata_proxies.status")
...
func (sIface *statusClient) Get(metadataProxyIdParam string, enforcementPointPathParam *string, segmentPathParam *string, sourceParam *string) (model.PolicyMetadataProxyStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.metadata_proxies.status", "get", inputDataValue, executionContext)
	var emptyOutput model.PolicyMetadataProxyStatus
