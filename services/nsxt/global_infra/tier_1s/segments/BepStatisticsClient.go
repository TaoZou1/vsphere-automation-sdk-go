...
func NewBepStatisticsClient(connector client.Connector) *bepStatisticsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.tier_1s.segments.bep_statistics")
...
func (bIface *bepStatisticsClient) Get(tier1IdParam string, segmentIdParam string, bridgeProfilePathParam string, enforcementPointPathParam *string) (model.BridgeEndpointStatistics, error) {
	...
	methodResult := bIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_1s.segments.bep_statistics", "get", inputDataValue, executionContext)
	var emptyOutput model.BridgeEndpointStatistics
