...
func NewCustomAttributesClient(connector client.Connector) *customAttributesClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.context_profiles.custom_attributes")
...
func (cIface *customAttributesClient) Create(policyAttributesParam model.PolicyAttributes, actionParam string) error {
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.context_profiles.custom_attributes", "create", inputDataValue, executionContext)
...
func (cIface *customAttributesClient) Patch(policyAttributesParam model.PolicyAttributes) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.context_profiles.custom_attributes", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), cIface.GetErrorBindingType(methodResult.Error().Name()))
