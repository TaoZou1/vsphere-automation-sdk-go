...
func NewProfilesClient(connector client.Connector) *profilesClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.profiles")
...
func (pIface *profilesClient) Get(profileIdParam string) (model.IdsProfile, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	pIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.profiles", "get", inputDataValue, executionContext)
	var emptyOutput model.IdsProfile
	if methodResult.IsSuccess() {
...
func (pIface *profilesClient) List(cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.IdsProfileListResult, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	pIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.profiles", "list", inputDataValue, executionContext)
	var emptyOutput model.IdsProfileListResult
	if methodResult.IsSuccess() {
