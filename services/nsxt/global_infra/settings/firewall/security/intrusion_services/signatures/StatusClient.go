...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.signatures.status")
...
func (sIface *statusClient) Get() (model.IdsSignatureStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.signatures.status", "get", inputDataValue, executionContext)
	var emptyOutput model.IdsSignatureStatus
