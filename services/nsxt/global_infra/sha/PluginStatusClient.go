...
func NewPluginStatusClient(connector client.Connector) *pluginStatusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sha.plugin_status")
...
func (pIface *pluginStatusClient) List(nodeIdParam string) (model.PerNodeShaPluginStatusListResult, error) {
	...
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sha.plugin_status", "list", inputDataValue, executionContext)
	var emptyOutput model.PerNodeShaPluginStatusListResult
