...
func NewNslookupClient(connector client.Connector) *nslookupClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.tier_0s.dns_forwarder.nslookup")
...
func (nIface *nslookupClient) Get(tier0IdParam string, addressParam *string, enforcementPointPathParam *string) (model.AggregatePolicyDnsAnswer, error) {
	...
	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.dns_forwarder.nslookup", "get", inputDataValue, executionContext)
	var emptyOutput model.AggregatePolicyDnsAnswer
