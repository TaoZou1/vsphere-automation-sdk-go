...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.tier_0s.dns_forwarder.status")
...
func (sIface *statusClient) Get(tier0IdParam string, enforcementPointPathParam *string) (model.AggregateDNSForwarderStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.dns_forwarder.status", "get", inputDataValue, executionContext)
	var emptyOutput model.AggregateDNSForwarderStatus
