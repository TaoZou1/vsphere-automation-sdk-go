...
func NewGatewayFirewallClient(connector client.Connector) *gatewayFirewallClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.gateway_firewall")
...
func (gIface *gatewayFirewallClient) List(tier0IdParam string, localeServicesIdParam string) (model.GatewayPolicyListResult, error) {
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.gateway_firewall", "list", inputDataValue, executionContext)
	var emptyOutput model.GatewayPolicyListResult
