...
func NewDadStateClient(connector client.Connector) *dadStateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.interfaces.dad_state")
...
func (dIface *dadStateClient) Get(tier0IdParam string, localeServiceIdParam string, interfaceIdParam string, enforcementPointPathParam *string) (model.InterfaceDADState, error) {
	...
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.interfaces.dad_state", "get", inputDataValue, executionContext)
	var emptyOutput model.InterfaceDADState
