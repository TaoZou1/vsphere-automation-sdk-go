...
func NewAggregatedClient(connector client.Connector) *aggregatedClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.drafts.aggregated")
...
func (aIface *aggregatedClient) Get(draftIdParam string) (model.Infra, error) {
	...
	methodResult := aIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.drafts.aggregated", "get", inputDataValue, executionContext)
	var emptyOutput model.Infra
