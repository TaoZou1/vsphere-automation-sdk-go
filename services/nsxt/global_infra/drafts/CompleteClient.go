...
func NewCompleteClient(connector client.Connector) *completeClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.drafts.complete")
...
func (cIface *completeClient) Get(draftIdParam string) (model.Infra, error) {
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.drafts.complete", "get", inputDataValue, executionContext)
	var emptyOutput model.Infra
