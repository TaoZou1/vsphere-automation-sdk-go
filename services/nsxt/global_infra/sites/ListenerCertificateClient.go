...
func NewListener_certificateClient(connector client.Connector) *listener_certificateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.listener_certificate")
...
func (lIface *listener_certificateClient) Get(addressParam string, portParam int64) (model.TlsListenerCertificate, error) {
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.listener_certificate", "get", inputDataValue, executionContext)
	var emptyOutput model.TlsListenerCertificate
