...
type SpanClient interface {
	...
	Get(orgIdParam string, projectIdParam string, intentPathParam string, sitePathParam *string) (model.Span, error)
}
...
func NewSpanClient(connector client.Connector) *spanClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.infra.span")
...
func (sIface *spanClient) Get(orgIdParam string, projectIdParam string, intentPathParam string, sitePathParam *string) (model.Span, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.span", "get", inputDataValue, executionContext)
	var emptyOutput model.Span
