...
type ManagerIpPoolsClient interface {
	...
	Get(orgIdParam string, projectIdParam string, managerIpPoolIdParam string) (model.IpAddressPool, error)
...
func NewManagerIpPoolsClient(connector client.Connector) *managerIpPoolsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.infra.manager_ip_pools")
...
func (mIface *managerIpPoolsClient) Get(orgIdParam string, projectIdParam string, managerIpPoolIdParam string) (model.IpAddressPool, error) {
	...
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.manager_ip_pools", "get", inputDataValue, executionContext)
	var emptyOutput model.IpAddressPool
...
func (mIface *managerIpPoolsClient) List(orgIdParam string, projectIdParam string, cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.IpAddressPoolListResult, error) {
	typeConverter := mIface.connector.TypeConverter()
	executionContext := mIface.connector.NewExecutionContext()
	...
	connectionMetadata["isStreamingResponse"] = false
	mIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.manager_ip_pools", "list", inputDataValue, executionContext)
	var emptyOutput model.IpAddressPoolListResult
	if methodResult.IsSuccess() {
