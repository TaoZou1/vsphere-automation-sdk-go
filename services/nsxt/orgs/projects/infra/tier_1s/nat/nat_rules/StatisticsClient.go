...
func NewStatisticsClient(connector client.Connector) *statisticsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.infra.tier_1s.nat.nat_rules.statistics")
...
func (sIface *statisticsClient) List(tier1IdParam string, natIdParam string, natRuleIdParam string, orgIdParam string, projectIdParam string, actionParam *string, containerClusterPathParam *string, enforcementPointPathParam *string) (model.PolicyNatRuleStatisticsListResult, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.tier_1s.nat.nat_rules.statistics", "list", inputDataValue, executionContext)
	var emptyOutput model.PolicyNatRuleStatisticsListResult
