...
func NewTagsClient(connector client.Connector) *tagsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.infra.domains.groups.tags")
...
func (tIface *tagsClient) Get(domainIdParam string, groupIdParam string, orgIdParam string, projectIdParam string) (model.GroupTagsList, error) {
	...
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.domains.groups.tags", "get", inputDataValue, executionContext)
	var emptyOutput model.GroupTagsList
