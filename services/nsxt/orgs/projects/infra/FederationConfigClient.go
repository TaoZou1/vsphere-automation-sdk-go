...
type FederationConfigClient interface {
	...
	Get(orgIdParam string, projectIdParam string) (model.FederationConfig, error)
}
...
func NewFederationConfigClient(connector client.Connector) *federationConfigClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.infra.federation_config")
...
func (fIface *federationConfigClient) Get(orgIdParam string, projectIdParam string) (model.FederationConfig, error) {
	...
	methodResult := fIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.federation_config", "get", inputDataValue, executionContext)
	var emptyOutput model.FederationConfig
