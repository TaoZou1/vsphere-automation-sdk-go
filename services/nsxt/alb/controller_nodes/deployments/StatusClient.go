...
type StatusClient interface {
	...
	Get(nodeIdParam string) (model.ALBControllerNodeVMDeploymentStatusReport, error)
}
...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.alb.controller_nodes.deployments.status")
...
func (sIface *statusClient) Get(nodeIdParam string) (model.ALBControllerNodeVMDeploymentStatusReport, error) {
	...
	if inputError != nil {
		var emptyOutput model.ALBControllerNodeVMDeploymentStatusReport
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.alb.controller_nodes.deployments.status", "get", inputDataValue, executionContext)
	var emptyOutput model.ALBControllerNodeVMDeploymentStatusReport
	if methodResult.IsSuccess() {
		...
		return output.(model.ALBControllerNodeVMDeploymentStatusReport), nil
	} else {
