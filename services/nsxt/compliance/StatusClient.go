...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.compliance.status")
...
func (sIface *statusClient) Get() (model.PolicyComplianceStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.compliance.status", "get", inputDataValue, executionContext)
	var emptyOutput model.PolicyComplianceStatus
