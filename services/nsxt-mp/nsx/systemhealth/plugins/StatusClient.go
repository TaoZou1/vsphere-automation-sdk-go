...
type StatusClient interface {
	...
	List(nodeIdParam string) (model.PluginStatusList, error)
}
...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.systemhealth.plugins.status")
...
func (sIface *statusClient) List(nodeIdParam string) (model.PluginStatusList, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.systemhealth.plugins.status", "list", inputDataValue, executionContext)
	var emptyOutput model.PluginStatusList
