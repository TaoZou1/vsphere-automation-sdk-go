...
type StatusClient interface {
	...
	Get(nodeIdParam string) (model.TnHyperbusStatus, error)
}
...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.systemhealth.transport_nodes.container.hyperbus.status")
...
func (sIface *statusClient) Get(nodeIdParam string) (model.TnHyperbusStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.systemhealth.transport_nodes.container.hyperbus.status", "get", inputDataValue, executionContext)
	var emptyOutput model.TnHyperbusStatus
