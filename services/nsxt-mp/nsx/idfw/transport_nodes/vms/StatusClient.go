...
type StatusClient interface {
	...
	List(transportNodeIdParam string) (model.IdfwVirtualMachineStatusListResult, error)
}
...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.idfw.transport_nodes.vms.status")
...
func (sIface *statusClient) List(transportNodeIdParam string) (model.IdfwVirtualMachineStatusListResult, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.idfw.transport_nodes.vms.status", "list", inputDataValue, executionContext)
	var emptyOutput model.IdfwVirtualMachineStatusListResult
