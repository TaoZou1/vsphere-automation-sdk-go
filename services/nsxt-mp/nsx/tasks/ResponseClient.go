...
func NewResponseClient(connector client.Connector) *responseClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.tasks.response")
...
func (rIface *responseClient) Get(taskIdParam string) (*data.StructValue, error) {
	...
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx.tasks.response", "get", inputDataValue, executionContext)
	var emptyOutput *data.StructValue
