...
type StatusClient interface {
	...
	Get(nodeIdParam string) (model.IntelligenceClusterNodeVMDeploymentStatusReport, error)
}
...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.intelligence.nodes.deployments.status")
...
func (sIface *statusClient) Get(nodeIdParam string) (model.IntelligenceClusterNodeVMDeploymentStatusReport, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.intelligence.nodes.deployments.status", "get", inputDataValue, executionContext)
	var emptyOutput model.IntelligenceClusterNodeVMDeploymentStatusReport
