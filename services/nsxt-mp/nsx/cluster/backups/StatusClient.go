...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.cluster.backups.status")
...
func (sIface *statusClient) Get() (model.CurrentBackupOperationStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.backups.status", "get", inputDataValue, executionContext)
	var emptyOutput model.CurrentBackupOperationStatus
