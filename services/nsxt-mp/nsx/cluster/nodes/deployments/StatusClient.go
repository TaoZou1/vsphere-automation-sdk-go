...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.cluster.nodes.deployments.status")
...
func (sIface *statusClient) Get(nodeIdParam string) (model.ClusterNodeVMDeploymentStatusReport, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.nodes.deployments.status", "get", inputDataValue, executionContext)
	var emptyOutput model.ClusterNodeVMDeploymentStatusReport
