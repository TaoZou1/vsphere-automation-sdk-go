...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.cluster.nodes.repo_sync.status")
...
func (sIface *statusClient) Get(nodeIdParam string) (model.RepoSyncStatusReport, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.nodes.repo_sync.status", "get", inputDataValue, executionContext)
	var emptyOutput model.RepoSyncStatusReport
