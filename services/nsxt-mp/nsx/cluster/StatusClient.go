...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.cluster.status")
...
func (sIface *statusClient) Get() (model.ClusterStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.status", "get", inputDataValue, executionContext)
	var emptyOutput model.ClusterStatus
