...
type StateClient interface {
	...
	Get(lportIdParam string) (model.LogicalPortState, error)
}
...
func NewStateClient(connector client.Connector) *stateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.logical_ports.state")
...
func (sIface *stateClient) Get(lportIdParam string) (model.LogicalPortState, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.logical_ports.state", "get", inputDataValue, executionContext)
	var emptyOutput model.LogicalPortState
