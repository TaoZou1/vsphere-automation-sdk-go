...
type StatusClient interface {
	...
	Get() (model.FederationStatus, error)
}
...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.sites.status")
...
func (sIface *statusClient) Get() (model.FederationStatus, error) {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	sv := bindings.NewStructValueBuilder(statusGetInputType(), typeConverter)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput model.FederationStatus
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.sites.status", "get", inputDataValue, executionContext)
	var emptyOutput model.FederationStatus
	if methodResult.IsSuccess() {
		...
		return output.(model.FederationStatus), nil
	} else {
