...
type StateClient interface {
	...
	Get(serviceIdParam string, serviceDeploymentIdParam string) (model.ConfigurationState, error)
}
...
func NewStateClient(connector client.Connector) *stateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.serviceinsertion.services.service_deployments.state")
...
func (sIface *stateClient) Get(serviceIdParam string, serviceDeploymentIdParam string) (model.ConfigurationState, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.serviceinsertion.services.service_deployments.state", "get", inputDataValue, executionContext)
	var emptyOutput model.ConfigurationState
