...
type StatusClient interface {
	...
	Get(serviceIdParam string, includeInstanceDetailsParam *bool, sourceParam *string, transportNodeIdsParam *string) (model.LbServiceStatus, error)
}
...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.loadbalancer.services.status")
...
func (sIface *statusClient) Get(serviceIdParam string, includeInstanceDetailsParam *bool, sourceParam *string, transportNodeIdsParam *string) (model.LbServiceStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.loadbalancer.services.status", "get", inputDataValue, executionContext)
	var emptyOutput model.LbServiceStatus
