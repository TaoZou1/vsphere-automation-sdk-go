...
type StateClient interface {
	...
	Get(serverIdParam string, bindingIdParam string, barrierIdParam *int64, requestIdParam *string) (model.ConfigurationState, error)
}
...
func NewStateClient(connector client.Connector) *stateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.dhcp.servers.static_bindings.state")
...
func (sIface *stateClient) Get(serverIdParam string, bindingIdParam string, barrierIdParam *int64, requestIdParam *string) (model.ConfigurationState, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.dhcp.servers.static_bindings.state", "get", inputDataValue, executionContext)
	var emptyOutput model.ConfigurationState
