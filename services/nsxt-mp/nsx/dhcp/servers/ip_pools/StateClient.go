...
type StateClient interface {
	...
	Get(serverIdParam string, poolIdParam string, barrierIdParam *int64, requestIdParam *string) (model.ConfigurationState, error)
}
...
func NewStateClient(connector client.Connector) *stateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.dhcp.servers.ip_pools.state")
...
func (sIface *stateClient) Get(serverIdParam string, poolIdParam string, barrierIdParam *int64, requestIdParam *string) (model.ConfigurationState, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.dhcp.servers.ip_pools.state", "get", inputDataValue, executionContext)
	var emptyOutput model.ConfigurationState
