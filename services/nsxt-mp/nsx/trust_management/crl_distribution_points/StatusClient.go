...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.trust_management.crl_distribution_points.status")
...
func (sIface *statusClient) Get(crlDistributionPointIdParam string) (model.CrlDistributionPointStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.trust_management.crl_distribution_points.status", "get", inputDataValue, executionContext)
	var emptyOutput model.CrlDistributionPointStatus
