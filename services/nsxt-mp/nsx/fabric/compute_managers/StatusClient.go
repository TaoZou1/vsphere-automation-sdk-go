...
type StatusClient interface {
	...
	Get(computeManagerIdParam string) (model.ComputeManagerStatus, error)
}
...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.fabric.compute_managers.status")
...
func (sIface *statusClient) Get(computeManagerIdParam string) (model.ComputeManagerStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.fabric.compute_managers.status", "get", inputDataValue, executionContext)
	var emptyOutput model.ComputeManagerStatus
