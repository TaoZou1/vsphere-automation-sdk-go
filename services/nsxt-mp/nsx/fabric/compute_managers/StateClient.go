...
type StateClient interface {
	...
	Get(computeManagerIdParam string) (model.ConfigurationState, error)
}
...
func NewStateClient(connector client.Connector) *stateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.fabric.compute_managers.state")
...
func (sIface *stateClient) Get(computeManagerIdParam string) (model.ConfigurationState, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.fabric.compute_managers.state", "get", inputDataValue, executionContext)
	var emptyOutput model.ConfigurationState
