...
type StatusClient interface {
	...
	Get(sessionIdParam string, sourceParam *string) (model.L2VPNSessionStatus, error)
}
...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.vpn.l2vpn.sessions.status")
...
func (sIface *statusClient) Get(sessionIdParam string, sourceParam *string) (model.L2VPNSessionStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.vpn.l2vpn.sessions.status", "get", inputDataValue, executionContext)
	var emptyOutput model.L2VPNSessionStatus
