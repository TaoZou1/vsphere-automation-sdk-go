...
type StateClient interface {
	...
	Get(sectionIdParam string, barrierIdParam *int64, requestIdParam *string) (model.FirewallSectionState, error)
}
...
func NewStateClient(connector client.Connector) *stateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.firewall.sections.state")
...
func (sIface *stateClient) Get(sectionIdParam string, barrierIdParam *int64, requestIdParam *string) (model.FirewallSectionState, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.firewall.sections.state", "get", inputDataValue, executionContext)
	var emptyOutput model.FirewallSectionState
