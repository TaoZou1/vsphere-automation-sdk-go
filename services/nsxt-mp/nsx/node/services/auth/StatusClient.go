...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx.node.services.auth.status")
...
func (sIface *statusClient) Get() (model.NodeServiceStatusProperties, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.node.services.auth.status", "get", inputDataValue, executionContext)
	var emptyOutput model.NodeServiceStatusProperties
