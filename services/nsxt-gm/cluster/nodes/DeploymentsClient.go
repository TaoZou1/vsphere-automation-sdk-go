...
type DeploymentsClient interface {
	...
	// @return com.vmware.nsx_global_policy.model.ClusterNodeVMDeploymentRequestList
	...
	//
	// @param nodeIdParam (required)
	// @return com.vmware.nsx_global_policy.model.ClusterNodeVMDeploymentRequest
	// @throws InvalidRequest  Bad Request, Precondition Failed
	// @throws Unauthorized  Forbidden
	// @throws ServiceUnavailable  Service Unavailable
	// @throws InternalServerError  Internal Server Error
...
func NewDeploymentsClient(connector client.Connector) *deploymentsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_global_policy.cluster.nodes.deployments")
...
func (dIface *deploymentsClient) Create(addClusterNodeVMInfoParam model.AddClusterNodeVMInfo) (model.ClusterNodeVMDeploymentRequestList, error) {
	...
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.cluster.nodes.deployments", "create", inputDataValue, executionContext)
	var emptyOutput model.ClusterNodeVMDeploymentRequestList
...
func (dIface *deploymentsClient) Delete(nodeIdParam string, forceDeleteParam *bool) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	dIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.cluster.nodes.deployments", "delete", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), dIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (dIface *deploymentsClient) Get(nodeIdParam string) (model.ClusterNodeVMDeploymentRequest, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	dIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.cluster.nodes.deployments", "get", inputDataValue, executionContext)
	var emptyOutput model.ClusterNodeVMDeploymentRequest
	if methodResult.IsSuccess() {
...
func (dIface *deploymentsClient) List() (model.ClusterNodeVMDeploymentRequestList, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	dIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.cluster.nodes.deployments", "list", inputDataValue, executionContext)
	var emptyOutput model.ClusterNodeVMDeploymentRequestList
	if methodResult.IsSuccess() {
