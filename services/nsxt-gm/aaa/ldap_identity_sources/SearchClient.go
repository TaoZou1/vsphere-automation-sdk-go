...
func NewSearchClient(connector client.Connector) *searchClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_global_policy.aaa.ldap_identity_sources.search")
...
func (sIface *searchClient) Create(ldapIdentitySourceIdParam string, filterValueParam string) (model.LdapIdentitySourceSearchResultList, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.aaa.ldap_identity_sources.search", "create", inputDataValue, executionContext)
	var emptyOutput model.LdapIdentitySourceSearchResultList
