...
type VersionWhitelistClient interface {
	...
	// @return com.vmware.nsx_global_policy.model.AcceptableComponentVersion
	...
	// @return com.vmware.nsx_global_policy.model.AcceptableComponentVersionList
	// @throws InvalidRequest  Bad Request, Precondition Failed
	// @throws Unauthorized  Forbidden
	// @throws ServiceUnavailable  Service Unavailable
	// @throws InternalServerError  Internal Server Error
...
func NewVersionWhitelistClient(connector client.Connector) *versionWhitelistClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_global_policy.upgrade.version_whitelist")
...
func (vIface *versionWhitelistClient) Get(componentTypeParam string) (model.AcceptableComponentVersion, error) {
	...
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.upgrade.version_whitelist", "get", inputDataValue, executionContext)
	var emptyOutput model.AcceptableComponentVersion
...
func (vIface *versionWhitelistClient) List() (model.AcceptableComponentVersionList, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	vIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.upgrade.version_whitelist", "list", inputDataValue, executionContext)
	var emptyOutput model.AcceptableComponentVersionList
	if methodResult.IsSuccess() {
...
func (vIface *versionWhitelistClient) Update(componentTypeParam string, versionListParam model.VersionList) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	vIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.upgrade.version_whitelist", "update", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), vIface.GetErrorBindingType(methodResult.Error().Name()))
