...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_global_policy.compliance.status")
...
func (sIface *statusClient) Get() (model.PolicyComplianceStatus, error) {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	sv := bindings.NewStructValueBuilder(statusGetInputType(), typeConverter)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput model.PolicyComplianceStatus
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.compliance.status", "get", inputDataValue, executionContext)
	var emptyOutput model.PolicyComplianceStatus
	if methodResult.IsSuccess() {
		...
		return output.(model.PolicyComplianceStatus), nil
	} else {
