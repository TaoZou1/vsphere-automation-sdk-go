...
func NewDadStateClient(connector client.Connector) *dadStateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.tier_1s.locale_services.interfaces.dad_state")
...
func (dIface *dadStateClient) Get(tier1IdParam string, localeServiceIdParam string, interfaceIdParam string, enforcementPointPathParam *string) (model.InterfaceDADState, error) {
	...
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.tier_1s.locale_services.interfaces.dad_state", "get", inputDataValue, executionContext)
	var emptyOutput model.InterfaceDADState
