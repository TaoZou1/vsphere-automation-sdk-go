...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.tier_0s.locale_services.inter_site.status")
...
func (sIface *statusClient) Get(tier0IdParam string, localeServicesIdParam string, cursorParam *string, edgePathParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.PolicyEdgeClusterInterSiteStatus, error) {
	...
	if inputError != nil {
		var emptyOutput model.PolicyEdgeClusterInterSiteStatus
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.tier_0s.locale_services.inter_site.status", "get", inputDataValue, executionContext)
	var emptyOutput model.PolicyEdgeClusterInterSiteStatus
	if methodResult.IsSuccess() {
		...
		return output.(model.PolicyEdgeClusterInterSiteStatus), nil
	} else {
