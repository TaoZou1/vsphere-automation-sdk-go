...
func NewStatisticsClient(connector client.Connector) *statisticsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.tier_0s.dns_forwarder.statistics")
...
func (sIface *statisticsClient) Get(tier0IdParam string, enforcementPointPathParam *string) (model.AggregateDNSForwarderStatistics, error) {
	...
	if inputError != nil {
		var emptyOutput model.AggregateDNSForwarderStatistics
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.tier_0s.dns_forwarder.statistics", "get", inputDataValue, executionContext)
	var emptyOutput model.AggregateDNSForwarderStatistics
	if methodResult.IsSuccess() {
		...
		return output.(model.AggregateDNSForwarderStatistics), nil
	} else {
