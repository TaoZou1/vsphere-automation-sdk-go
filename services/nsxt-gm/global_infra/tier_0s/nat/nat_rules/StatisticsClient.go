...
func NewStatisticsClient(connector client.Connector) *statisticsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.tier_0s.nat.nat_rules.statistics")
...
func (sIface *statisticsClient) List(tier0IdParam string, natIdParam string, natRuleIdParam string, actionParam *string, containerClusterPathParam *string, enforcementPointPathParam *string) (model.PolicyNatRuleStatisticsListResult, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.tier_0s.nat.nat_rules.statistics", "list", inputDataValue, executionContext)
	var emptyOutput model.PolicyNatRuleStatisticsListResult
