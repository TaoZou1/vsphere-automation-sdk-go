...
import (
	...
	"github.com/TaoZou1/vsphere-automation-sdk-go/services/vmc/autoscaler/model"
)
...
type RemediationPolicyClient interface {
	...
	// Get the Remediation policy on every cluster in an SDDC.
	//
	...
	Get(orgParam string, sddcParam string) ([]model.RemediationClusterInfo, error)

	// Change the current Remediation policy on every cluster in an SDDC.
	//
	// @param orgParam org identifier (required)
	// @param sddcParam Sddc identifier (required)
	...
	// @throws InvalidRequest  Invalid action or bad argument
	// @throws Unauthorized  Forbidden
	Patch(orgParam string, sddcParam string, remediationPolicyPatchParam model.RemediationPolicyPatch) (model.Task, error)
}
