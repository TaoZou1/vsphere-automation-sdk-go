...
func NewServiceClient(connector client.Connector) *serviceClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.vapi.metadata.authentication.service")
...
func (sIface *serviceClient) List() ([]string, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.vapi.metadata.authentication.service", "list", inputDataValue, executionContext)
	var emptyOutput []string
...
func (sIface *serviceClient) Get(serviceIdParam string) (ServiceInfo, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.vapi.metadata.authentication.service", "get", inputDataValue, executionContext)
	var emptyOutput ServiceInfo
	if methodResult.IsSuccess() {
