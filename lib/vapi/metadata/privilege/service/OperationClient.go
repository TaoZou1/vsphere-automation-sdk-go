...
func NewOperationClient(connector client.Connector) *operationClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.vapi.metadata.privilege.service.operation")
...
func (oIface *operationClient) Get(serviceIdParam string, operationIdParam string) (privilege.OperationInfo, error) {
	typeConverter := oIface.connector.TypeConverter()
	executionContext := oIface.connector.NewExecutionContext()
